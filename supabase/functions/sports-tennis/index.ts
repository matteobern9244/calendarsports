...
function parseInfobox(html: string): Record<string, string> {
  ...
  const re =
    /class="infobox-label"[^>]*>([\s\S]{1,200}?)<\/th>\s*<td class="infobox-data"[^>]*>([\s\S]{0,500}?)<\/td>/g;
...
function parseSinottico(html: string): Record<string, string> {
  ...
  const re =
    /<tr[^>]*>\s*<th[^>]*>([\s\S]{1,300}?)<\/th>\s*<td[^>]*>([\s\S]{0,800}?)<\/td>\s*<\/tr>/g;
  let m: RegExpExecArray | null;
...
function parseStatsUpdatedAt(html: string): string | null {
  const m = html.match(
    /Statistiche\s+aggiornate\s+al[\s\S]{0,40}?(\d{1,2})\s+([a-zA-Zàèéìòù]+)\s+(\d{4})/i,
  );
  if (!m) return null;
...
const PHOTO_URL =
  "https://upload.wikimedia.org/wikipedia/commons/thumb/4/48/Jannik_Sinner_US_Open_2025_%28cropped%29.jpg/500px-Jannik_Sinner_US_Open_2025_%28cropped%29.jpg";
...
async function getPlayerInfo() {
  ...
  const seasonHtml = await fetchWiki(
    "https://en.wikipedia.org/wiki/2026_Jannik_Sinner_tennis_season",
  );
  ...
  if ((!careerRecord || !careerTitlesRaw) && html) {
    ...
    if (singlesIdx >= 0) {
      const slice = html.substring(
        singlesIdx,
        doublesIdx > singlesIdx ? doublesIdx : singlesIdx + 4000,
      );
      if (!careerRecord) {
...
function parseTournamentHeader(cellHtml: string): {
  ...
  const locMatch = afterName.match(
    /^\s*([A-Z][\w\s,'-]+?)\s+(?:Grand Slam|ATP|Tour Finals|Davis|Hard|Clay|Grass)/,
  );
...
async function getSeasonData() {
  ...
  if (tableStart < 0 || tableEnd < 0)
    return { matches: [], tournaments: [], curatedAppended: false };
  ...
  const tournamentMap = new Map<
    string,
    { header: ReturnType<typeof parseTournamentHeader>; matches: MatchRow[] }
  >();
  ...
  for (const row of rows) {
    ...
    if (cells.length >= 6 && currentTour) {
      ...
      const result: "V" | "S" | null = resultText.includes("win")
        ? "V"
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (pageSizeParam !== null && !/^\d+$/.test(pageSizeParam)) {
      return new Response(
        JSON.stringify({ success: false, error: "Parametro pageSize non valido" }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
    ...
    switch (action) {
      ...
      default:
        return new Response(
          JSON.stringify({
            success: false,
            error: "Azione non valida. Usa: player-info, schedule, results, next-event",
          }),
          {
            status: 400,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
