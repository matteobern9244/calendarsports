...
| `src/components/team/matchPhaseConsistency.test.tsx`      | che card e riga di calendario dicano cose diverse sulla stessa partita: «PROSSIMA PARTITA» accanto a «IN DIRETTA» non era sbagliato in nessuna delle due viste, solo nelle due insieme                        |
| `src/test/tooling/matchStatusMirror.test.ts`              | la divergenza fra il vocabolario degli stati in `src/lib/matchPhase.ts` e quello in `supabase/functions/sports-football/matchStatus.ts`: uno stato aggiunto da una parte sola nasconde o inventa un punteggio |
...
3. **Un guardiano che scansiona un elenco fisso di file invecchia male.**
   ...
   finché qualcuno non la aggiunge. È un limite noto, non un disegno. Vale anche
   per `queryKeys.test.ts`, che sorveglia un elenco di file: un hook nuovo che
   scrive chiavi a mano non fa rumore finché non entra in quell'elenco. La
   contromisura è aggiungerlo **prima** di scrivere il codice, così il test è
   rosso subito.
