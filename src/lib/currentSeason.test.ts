...
describe("currentSeason helpers", () => {
  ...
  it("Juventus: aprile 2026 -> 2025 (Serie A 2025/26)", () => {
    expect(getCurrentFootballSeason(new Date("2026-04-15T12:00:00Z"))).toBe(2025);
  });

  it("Juventus: gennaio 2026 -> 2025", () => {
    expect(getCurrentFootballSeason(new Date("2026-01-10T12:00:00Z"))).toBe(2025);
  });

  it("Juventus: giugno 2026 -> 2025 (stagione finita ma cutoff luglio)", () => {
    expect(getCurrentFootballSeason(new Date("2026-06-30T12:00:00Z"))).toBe(2025);
  });

  it("Juventus: luglio 2026 -> 2026 (nuova stagione 2026/27)", () => {
    expect(getCurrentFootballSeason(new Date("2026-07-01T12:00:00Z"))).toBe(2026);
  });

  it("Juventus: settembre 2026 -> 2026", () => {
    expect(getCurrentFootballSeason(new Date("2026-09-15T12:00:00Z"))).toBe(2026);
  });

  it("formatFootballSeasonLabel: 2025 -> 2025/26", () => {
    expect(formatFootballSeasonLabel(2025)).toBe("2025/26");
  });

  it("formatFootballSeasonLabel: 2099 -> 2099/00", () => {
    expect(formatFootballSeasonLabel(2099)).toBe("2099/00");
  });

  it("formatFootballSeasonLabel: 2009 -> 2009/10", () => {
    expect(formatFootballSeasonLabel(2009)).toBe("2009/10");
  });
});
