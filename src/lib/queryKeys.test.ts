...
describe("queryKeys", () => {
  ...
  it("il calendario di una squadra senza paginazione e' una voce di cache distinta", () => {
    expect(queryKeys.football.calendar("juventus", 2026)).toEqual([
      "football",
      "calendar",
      "juventus",
      2026,
    ...
    expect(queryKeys.football.calendar("juventus", 2026, 1, 12)).not.toEqual(
      queryKeys.football.calendar("juventus", 2026),
    );
  });
  ...
  it("il calendario di due squadre sono due voci di cache diverse", () => {
    ...
    expect(queryKeys.football.calendar("napoli", 2026, 1, 12)).not.toEqual(
      queryKeys.football.calendar("juventus", 2026, 1, 12),
    );
    expect(queryKeys.football.info("napoli", 2026)).not.toEqual(
      queryKeys.football.info("juventus", 2026),
    );
    expect(queryKeys.football.calendarAll("napoli", 2026, 3)).not.toEqual(
      queryKeys.football.calendarAll("juventus", 2026, 3),
    );
  });
  ...
  it("la classifica NON porta la squadra", () => {
    ...
    expect(queryKeys.football.standings(2026)).toEqual(["football", "standings", 2026]);
  });
